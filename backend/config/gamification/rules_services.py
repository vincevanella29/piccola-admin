# config/gamification/rules_services.py
...
from config.gamification.rules import (
    list_rule_templates as rules_list_templates,
    list_rule_templates_enriched as rules_list_templates_enriched,  # ⬅️ enriched siempre
)
...
# ---- Rule templates (predefined) ----

def list_rule_templates_service() -> Dict[str, Any]:
    """
    Lista templates **enriquecidos SIEMPRE** (catálogos incluidos).
    No requiere parámetros: el período y opciones se infieren en rules.py.
    """
    try:
        templates = rules_list_templates_enriched()
        return {"ok": True, "templates": templates}
    except Exception as e:
        logger.exception("Error listing rule templates (enriched). Fallback to plain list.")
        try:
            templates = rules_list_templates()
            return {"ok": True, "templates": templates}
        except Exception as ee:
            logger.exception("Error listing rule templates (plain)")
...
def validate_and_save_rule_from_template(payload: Dict[str, Any]) -> Dict[str, Any]:
    ...
    for k, meta in req.items():
        ...
        if meta.get("type") == "number":
            ...
            params[k] = int(v) if float(v).is_integer() else v
