...
def main():
    ...
    try:
        ...
        col = db['rentabilidad_producto_mtz']
        logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
        for mesano in periodos:
            API_URL = f"http://192.168.4.117:8000/api/rentabilidad-por-producto-mtz?mesano={mesano}"
            logging.info(f"Consultando API rentabilidad por producto MTZ para mes/año {mesano}...")
            try:
