...
def main():
    ...
    try:
        ...
        col = db['rentabilidad_producto_locales']
        logging.basicConfig(level=logging.INFO, format='%(asctime)s %(levelname)s: %(message)s')
        for mesano in periodos:
            API_URL = f"http://192.168.4.117:8000/api/rentabilidad-por-producto?mesano={mesano}"
            logging.info(f"Consultando API rentabilidad por producto locales para mes/año {mesano}...")
            try:
                ...
                if resp.status_code == 200:
                    ...
                    if data:
                        ...
                        for item in data:
                            ...
                            if not key:
                                continue
                            # El _id será unico por producto y mesano
                            item["_id"] = f"{key}_{mesano}"
                            item["mesano"] = mesano
