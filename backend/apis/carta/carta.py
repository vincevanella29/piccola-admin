...
# Convention: pizza.webp → pizza_thumb.webp (same R2 bucket, generated by r2_upload.py)

def _thumb_url(url: str) -> str:
    """Derive thumbnail URL from an original image URL."""
    if not url or not isinstance(url, str) or '_thumb.' in url:
        return ''
    m = re.match(r'^(.+)(\.[\w]+)$', url)
    return f"{m.group(1)}_thumb.webp" if m else ''


def _inject_thumb_fields(fields: dict) -> dict:
    """
    Auto-compute media_thumb from media_r2/media_images before saving to MongoDB.
    Called on every product create/update so the thumb URL is always in sync.
    """
    # From media_images list
    imgs = fields.get('media_images')
    if isinstance(imgs, list) and imgs:
        fields['media_thumb'] = _thumb_url(imgs[0])
        fields['media_images_thumb'] = [_thumb_url(u) for u in imgs if u]
    # From single media_r2
    elif fields.get('media_r2'):
        fields['media_thumb'] = _thumb_url(fields['media_r2'])
    return fields
...
async def create_product(data: ProductCreate, user: dict = Depends(_require_catalog_access)):
    payload = _inject_thumb_fields(data.dict())
    pid = await product_svc.create_product(payload)
...
async def update_product(product_id: str, data: ProductUpdate, user: dict = Depends(_require_catalog_access)):
    update_fields = _inject_thumb_fields(data.dict(exclude_unset=True))
...
async def upload_product_image(file: UploadFile = File(...), user: dict = Depends(_require_catalog_access)):
    try:
        ...
        return {"url": url, "thumb_url": _thumb_url(url)}
...
async def organize_product_media(product_id: str, req: OrganizeMediaRequest, user: dict = Depends(_require_catalog_access)):
    try:
        ...
        # Also save thumb URLs for the organized images
        if req.images:
            thumb_fields = _inject_thumb_fields({"media_images": req.images})
            await product_svc.update_product(product_id, thumb_fields)
        return result
    except ValueError as e:
        raise HTTPException(status_code=400, detail=str(e))
