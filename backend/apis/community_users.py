...
async def create_or_update_profile(
    ...
    existing_profile = db.user_profiles.find_one({"wallet": wallet})
    ...
    updated_profile = db.user_profiles.find_one({"wallet": wallet})
...
async def get_profile(user: dict = Depends(verify_session)):
    ...
    profile = db.user_profiles.find_one({"wallet": wallet})
    if not profile:
...
async def toggle_field(data: ToggleUpdate, user: dict = Depends(verify_session)):
    ...
    drip_result = None
    # Si el toggle es subscribe_news, sincroniza con Drip
    if data.field == "subscribe_news":
        # Busca el email del usuario en Mongo
        profile = db.user_profiles.find_one({"wallet": wallet})
        ...
        if user_email:
            ...
            if DRIP_ACCOUNT_ID and DRIP_API_KEY:
                url = f"https://api.getdrip.com/v2/{DRIP_ACCOUNT_ID}/subscribers"
                auth = (DRIP_API_KEY, '')
                # Subscribe: solo actualiza el estado a active
                # Añadir o quitar tag 'user_tokens' según suscripción
                tags = ["user_tokens"] if data.value else []
                # Si es suscripción, reactiva y suscribe globalmente
                subscriber_data = {"email": user_email, "tags": tags}
                if data.value:
                    subscriber_data["status"] = "active"
                    subscriber_data["subscribed"] = True
                    subscriber_data["remove"] = False
                else:
                ...
                try:
                    ...
                    drip_result = response.json() if response.text else {"subscribed": True}
                except Exception as e:
                    drip_result = {"error": str(e)}
                    try:
                        drip_result["drip_response"] = response.text
                    except Exception:
                        pass
            else:
                drip_result = {"error": "Drip config missing"}
        else:
            drip_result = {"error": "User email not found"}
    ...
    return {"success": True, "field": data.field, "value": data.value, "drip_result": drip_result}
...
async def update_profile_image(
    ...
    filename = f"profile_{wallet}_{int(current_time.timestamp())}.{ext}"
    url = upload_profile_image_to_r2(profile_image.file, filename)
    result = db.user_profiles.update_one(
        {"wallet": wallet},
        {"$set": {"profile_image_url": url, "updated_at": current_time}}
    )
    if result.matched_count == 0:
...
async def update_favorite_location(data: FavoriteLocationUpdate, user: dict = Depends(verify_session)):
    ...
    result = db.user_profiles.update_one(
        {"wallet": wallet},
        {"$set": {"favorite_location": data.location_id, "updated_at": current_time}}
    )
...
async def update_product_like(data: ProductLikeUpdate, user: dict = Depends(verify_session)):
    ...
    result = db.user_profiles.update_one(
        {"wallet": wallet},
        {"$set": {update_key: data.like, "updated_at": current_time}}
    )
