...
def get_user_rut_and_local(user: dict) -> tuple[str, Optional[str]]:
    ...
    worker = TRABAJADORES_COLL.find_one({"rut": rut})
    return rut, worker.get("sucursal") if worker else None

def _get_ranks_sync(rut: str, group_key: dict, periods: Optional[List[str]] = None) -> dict:
    ...
    rows = list(KPI_EMPLEADO_COLL.aggregate(pipeline, allowDiskUse=True))
...
async def mi_ventas_total(user: dict = Depends(verify_session)):
    ...
    if not KPI_EMPLEADO_COLL.find_one({"rut": rut}):
        return {"rut": rut, "local": local_actual, "mi_total": 0, "puesto_empresa": 0, "puesto_local": 0}

    user_total_data = list(KPI_EMPLEADO_COLL.aggregate([
        {"$match": {"rut": rut}},
    ...
    ranks = list(KPI_EMPLEADO_COLL.aggregate(pipeline_ranks, allowDiskUse=True))
...
async def mi_ventas_por_periodo(
    ...
    try:
        results = list(KPI_EMPLEADO_COLL.aggregate(pipeline))
...
async def mi_ventas_detalle_productos_enriquecido(
    ...
    most_frequent_local_list = list(KPI_EMPLEADO_COLL.aggregate([
    ...
    main_pipeline = [
        ...
        {"$facet": {
            ...
            "ventas_local": [ {"$match": {"local": primary_local, "periodo": {"$in": current_periods}, "es_competidor": True}}, {"$group": {"_id": {"familia": "$familia_norm", "subfamilia": "$subfamilia_norm", "rut": "$rut"}, "total": {"$sum": "$sales_by_category.total"}}}, {"$group": {"_id": {"familia": "$_id.familia", "subfamilia": "$_id.subfamilia"}, "promedio": {"$avg": "$total"}, "top": {"$max": "$total"}}} ],
            "ventas_empresa": [ {"$match": {"periodo": {"$in": current_periods}, "es_competidor": True}}, {"$group": {"_id": {"familia": "$familia_norm", "subfamilia": "$subfamilia_norm", "rut": "$rut"}, "total": {"$sum": "$sales_by_category.total"}}}, {"$group": {"_id": {"familia": "$_id.familia", "subfamilia": "$_id.subfamilia"}, "promedio": {"$avg": "$total"}, "top": {"$max": "$total"}}} ],
            "ventas_local_familia": [ {"$match": {"local": primary_local, "periodo": {"$in": current_periods}, "es_competidor": True}}, {"$group": {"_id": {"familia": "$familia_norm", "rut": "$rut"}, "total": {"$sum": "$sales_by_category.total"}}}, {"$group": {"_id": "$_id.familia", "promedio": {"$avg": "$total"}, "top": {"$max": "$total"}}} ],
            "ventas_empresa_familia": [ {"$match": {"periodo": {"$in": current_periods}, "es_competidor": True}}, {"$group": {"_id": {"familia": "$familia_norm", "rut": "$rut"}, "total": {"$sum": "$sales_by_category.total"}}}, {"$group": {"_id": "$_id.familia", "promedio": {"$avg": "$total"}, "top": {"$max": "$total"}}} ]
        }}
    ]
    try:
        ...
        tasks = [
            asyncio.to_thread(lambda: list(KPI_EMPLEADO_COLL.aggregate(main_pipeline, allowDiskUse=True))),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_subfamily, current_periods),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_subfamily, previous_periods),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_subfamily),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_family, current_periods),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_family, previous_periods),
            asyncio.to_thread(_get_ranks_sync, rut, group_key_family)
        ]
