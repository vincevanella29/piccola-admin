...
import { Building2, KeyRound, CheckCircle2, AlertCircle, Copy, Check, Fingerprint, ArrowRight, ShieldCheck, Code, Terminal } from 'lucide-react';
...
axios.post('https://api.vanellix.com/api/public/b2b/distribute', payload, { headers })
    ...
    return (
      <div className="max-w-2xl mx-auto py-16 px-4">
        <motion.div
          ...
          className="bg-light-surface dark:bg-dark-surface p-10 rounded-3xl border border-light-border/30 dark:border-dark-border/30 shadow-xl text-center"
        >
          {/* Glow background */}
  ...
  if (!partner) {
    return (
      <div className="max-w-2xl mx-auto py-12 px-4">
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-light-surface dark:bg-dark-surface p-8 rounded-3xl border border-light-border/30 dark:border-dark-border/30 shadow-xl">
          <div className="flex items-center gap-4 mb-8">
            <div className="w-12 h-12 rounded-xl bg-matrix-green/10 flex items-center justify-center text-matrix-green">
              <Building2 size={24} />
            </div>
            <div>
              <h1 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary">{t('b2b.register_title')}</h1>
              <p className="text-light-text-secondary dark:text-dark-text-secondary">{t('b2b.register_desc')}</p>
            </div>
          </div>
          ...
          <form onSubmit={handleRegister} className="space-y-4">
            <div>
              <label className="block text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-1">{t('b2b.company_name')}</label>
              <input required type="text" value={formData.company_name} onChange={(e) => setFormData({ ...formData, company_name: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-light-surface-secondary/50 dark:bg-dark-surface-secondary/50 border border-light-border/50 dark:border-dark-border/50 focus:ring-2 focus:ring-matrix-green outline-none text-light-text-primary dark:text-dark-text-primary" />
            </div>
            <div>
              <label className="block text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-1">{t('b2b.contact_email')}</label>
              <input required type="email" value={formData.contact_email} onChange={(e) => setFormData({ ...formData, contact_email: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-light-surface-secondary/50 dark:bg-dark-surface-secondary/50 border border-light-border/50 dark:border-dark-border/50 focus:ring-2 focus:ring-matrix-green outline-none text-light-text-primary dark:text-dark-text-primary" />
            </div>
            <div>
              <label className="block text-sm font-medium text-light-text-primary dark:text-dark-text-primary mb-1">{t('b2b.contact_phone')}</label>
              <input type="text" value={formData.contact_phone} onChange={(e) => setFormData({ ...formData, contact_phone: e.target.value })} className="w-full px-4 py-3 rounded-xl bg-light-surface-secondary/50 dark:bg-dark-surface-secondary/50 border border-light-border/50 dark:border-dark-border/50 focus:ring-2 focus:ring-matrix-green outline-none text-light-text-primary dark:text-dark-text-primary" />
            </div>
            <button type="submit" className="w-full py-4 mt-4 bg-matrix-green hover:opacity-90 text-white rounded-xl font-medium transition-opacity">
              {t('b2b.submit_btn')}
            </button>
          </form>
        </motion.div>
  ...
  if (partner.status === 'pending') {
    return (
      <div className="max-w-2xl mx-auto py-12 px-4">
        <motion.div initial={{ opacity: 0, scale: 0.95 }} animate={{ opacity: 1, scale: 1 }} className="bg-light-surface dark:bg-dark-surface p-8 rounded-3xl border border-light-border/30 dark:border-dark-border/30 shadow-xl text-center">
          <div className="w-20 h-20 mx-auto rounded-full bg-yellow-500/10 flex items-center justify-center text-yellow-500 mb-6">
            <AlertCircle size={40} />
          </div>
          <h2 className="text-2xl font-bold text-light-text-primary dark:text-dark-text-primary mb-2">{t('b2b.pending_title')}</h2>
          <p className="text-light-text-secondary dark:text-dark-text-secondary mb-6">{t('b2b.pending_desc')} <strong>{partner.company_name}</strong>. {t('b2b.pending_desc_suffix')}</p>
        </motion.div>
      </div>
    );
  }
    ...
    <div className="max-w-6xl mx-auto py-8 px-4 space-y-8">
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4 mb-8">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h1 className="text-3xl font-bold text-light-text-primary dark:text-dark-text-primary">{partner.company_name}</h1>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-3xl border border-light-border/30 dark:border-dark-border/30 shadow-sm">
          <h3 className="text-xl font-bold text-light-text-primary dark:text-dark-text-primary mb-6">{t('b2b.promotions_quota')}</h3>
                  ...
                  <div className="flex justify-between items-start mb-2">
                    <div>
                      ...
                      <div className="flex items-center gap-2 mt-0.5">
                        <p className="text-xs text-light-text-secondary dark:text-dark-text-secondary">Allocation ID: <span className="font-mono">{a._id}</span></p>
                        <button 
        ...
        <div className="bg-light-surface dark:bg-dark-surface p-6 rounded-3xl border border-light-border/30 dark:border-dark-border/30 shadow-sm space-y-6">
          ...
          {!partner.has_credentials && !newCredentials ? (
            <div className="p-6 rounded-2xl bg-matrix-green/5 border border-matrix-green/20 text-center">
              ...
              <h4 className="font-semibold text-light-text-primary dark:text-dark-text-primary mb-2">{t('b2b.generate_credentials_title')}</h4>
              <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary mb-6">{t('b2b.generate_credentials_desc')}</p>
            ...
            <div className="p-6 rounded-2xl bg-matrix-green/5 border border-matrix-green/20">
              <div className="flex items-start gap-4 mb-4">
                ...
                <div>
                  <h4 className="font-semibold text-light-text-primary dark:text-dark-text-primary mb-1">{t('b2b.active_credentials_title')}</h4>
                  <p className="text-sm text-light-text-secondary dark:text-dark-text-secondary">{t('b2b.active_credentials_desc')}</p>
                </div>
              </div>
          ...
          <div className="space-y-6">
            <h4 className="text-lg font-bold text-light-text-primary dark:text-gray-200 border-b border-light-border/50 dark:border-white/10 pb-2">
              {t('b2b.integration_guide', 'Guía de Integración Rápida')}
            </h4>
