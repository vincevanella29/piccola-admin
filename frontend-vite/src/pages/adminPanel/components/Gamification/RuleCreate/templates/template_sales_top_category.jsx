...
/* ==============================================
 * Top Ventas por Categoría / Producto
 * — Si level=product => selector de 1+ productos
 * — Si level=family  => selector de 1+ familias
 * — Si level=subfamily => selector de 1+ subfamilias
 * Guarda en params:
 *   - level: 'family' | 'subfamily' | 'product'
 *   - selected_keys: string[]   (keys de familia/subfamilia o códigos SKU de productos)
 ...
 * ============================================== */
export const salesTopCategoryTemplate = {
  key: 'sales_top_category',
  name: 'Top Ventas por Categoría/Producto',
  description:
    'Premia a quienes lideran ventas por familia, subfamilia o producto (monto o cantidad), a nivel local o empresa.',
  category: 'sales',
  period: 'month',
  data_sources: [
    {
      ...
      filter:
        'Descompone categorías/ítems y agrupa por RUT (y local); ordena por total o cantidad para calcular ranking.',
    },
  ],
  required_params: {
    level: {
      ...
      default: 'family',
      description: 'Nivel a evaluar (familia, subfamilia o producto).',
    },
    metric: {
      type: 'select',
  ...
  Component: function SalesTopCategoryComponent({ formData, setFormData, errors, t }) {
    ...
    const productOptions = useMemo(() => {
      ...
      // value = code (SKU), label legible con nombre
      return list.map(p => ({
        value: p.code,
        label: `${p.code} — ${p.name}`,
        meta: p,
    ...
    return (
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-6 gap-4">
          ...
          <div className="flex flex-col gap-1.5 md:col-span-2">
            ...
            >
              ...
              <option value="product">product (producto)</option>
            </select>
          </div>
          ...
          <div className="flex flex-col gap-1.5 md:col-span-4 bg-light-surface dark:bg-dark-surface-secondary dark:border-dark-border/20 border border-dark-border/20 rounded-lg p-3">
            <label className="text-sm font-medium">
              {level === 'product' ? 'Productos (1 o más)' :
               level === 'subfamily' ? 'Subfamilias (1 o más)' :
               'Familias (1 o más)'}
            </label>
            <Select
              isMulti
        ...
        <p className="text-xs text-dark-text-secondary">
          ...
          <code> selected_labels</code>. Usa <code>selected_keys</code> para filtrar en tu pipeline (familia/subfamilia = key; producto = code).
        </p>
      </div>
    );
  },
