...
const AdminLightTheme = ({ appState, onConfigChange, formData, isLoading }) => {
  ...
  const handleColorChange = (colorKey, color) => {
    onConfigChange({
      colors: {
        ...formData.colors,
        light: { ...formData.colors.light, [colorKey]: color },
      },
    });
  };
  ...
  const handleUseDefault = (colorKey) => {
    onConfigChange({
      colors: {
        ...formData.colors,
        light: { ...formData.colors.light, [colorKey]: DEFAULT_COLORS[colorKey] },
      },
    });
  };
          ...
          return (
            <div key={colorKey} className="relative">
              <label className="block text-sm font-medium text-light-text-secondary dark:text-dark-text-secondary mb-2 flex items-center gap-1">
                {t(`admin.color_levels.light_${field}`)}
                <InfoTooltip text={t(`admin.color_levels.light_${field}_tooltip`)} />
              </label>
              <div className="flex items-center gap-2">
                <div
                  ...
                  style={{ backgroundColor: formData.colors.light[colorKey] || DEFAULT_COLORS[colorKey] }}
                  onClick={() => setActivePicker(activePicker === colorKey ? null : colorKey)}
                />
                <input
                  type="text"
                  value={formData.colors.light[colorKey] || ''}
                  onChange={(e) => handleColorChange(colorKey, e.target.value)}
                  placeholder={DEFAULT_COLORS[colorKey] || '#FFFFFF'}
              ...
              {activePicker === colorKey && (
                ...
                >
                  <div onClick={(e) => e.stopPropagation()}>
                    <ModernColorPicker
                      value={formData.colors.light[colorKey] || DEFAULT_COLORS[colorKey]}
