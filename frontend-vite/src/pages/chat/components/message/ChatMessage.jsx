    ...
    >
      {isLoading && (messages?.length || 0) === 0 ? (
        ...
        messages.map((m, idx) => {
          ...
          // ONE source: backend resolves avatar from trabajadores_vpn
          const avatarUrl = m?.sender_avatar_url || participant.avatar_url || participant.profile_image_url || '';
          const senderSeccion = m?.sender_seccion || participant?.seccion || participant?.profile?.seccion || '';
          ...
          if (idx < 5 || !avatarUrl) {
            console.log('[AVATAR MSG]', {
              ...
              participant_profile_image: participant.profile_image_url,
              FINAL: avatarUrl,
            });
          }
                ...
                <div className="flex-1 min-w-0 max-w-full overflow-x-hidden">
                  <div className="flex items-center justify-between gap-2 mb-1">
                    <button
                      ...
                      style={{ color: isAssistant ? '#ec4899' : (isMe ? undefined : sectionColor.color) }}
                      onClick={() => !isAssistant && handleShowProfile(participant)}
                      title={displayName}
