...
const CameraControls = ({
  ...
  return (
    <div className="absolute bottom-0 left-0 right-0 z-30 bg-gradient-to-t from-black/70 to-transparent pb-[env(safe-area-inset-bottom)] pt-8">
      <div className="max-w-md mx-auto px-6 flex items-center justify-between">
        ...
        <button
          ...
          className="h-20 w-20 rounded-full bg-white/20 border-4 border-white flex items-center justify-center text-white active:scale-90 transition-transform ring-4 ring-black/30 disabled:opacity-50"
        >
          {classifying ? (
      ...
      <div className="mt-4 text-center">
         ...
         >
           <Zap className={`h-4 w-4 ${autoMode ? 'text-matrix-green' : 'text-white/60'}`} />
...
const ControlButton = ({ onClick, disabled, active, icon, label }) => (
  <button
    ...
    disabled={disabled}
    aria-label={label}
    className={`h-12 w-12 rounded-full flex items-center justify-center text-white active:scale-90 transition-all duration-200 disabled:opacity-30 ${
