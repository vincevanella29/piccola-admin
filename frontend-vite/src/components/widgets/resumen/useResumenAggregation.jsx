...
export default function useResumenAggregation({
  ...
  const aggregateByGranularity = React.useCallback(
    (rows) => {
      ...
      // Arma salida + clima opcional para tooltip
      return Array.from(map.entries()).map(([date, { value, details, startDate, endDate }]) => {
        ...
        return { date, value, details, startDate, endDate, _climaSummary: climaSummary };
      });
    },
          ...
          callbacks: {
            ...
            afterBody: (items) => {
              ...
              if (mode === 'aggregate') {
                ...
                if (!sumActual?.hasClima && !sumPrevio?.hasClima) return;
