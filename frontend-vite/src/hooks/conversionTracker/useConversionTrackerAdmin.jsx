...
const useConversionTrackerAdmin = ({ token, account, autoLoad = true } = {}) => {
  ...
  const [ecosystemProviders, setEcosystemProviders] = useState([]);
  ...
  const fetchRealtimeAnalytics = useCallback(async (providerId = null) => {
    try {
      setLoading(true);
      const res = await conversionTrackerApi.getRealtimeAnalytics({ providerId, token, account });
      setRealtimeData(res);
      return res;
    } catch (err) {
      console.warn("[GA4] Realtime fetch skipped:", err?.message || err);
  ...
  const fetchAnalyticsProviders = useCallback(async () => {
    try {
      const data = await conversionTrackerApi.listAnalyticsProviders({ token, account });
      setAnalyticsProviders(Array.isArray(data) ? data : []);
      return data;
    } catch (e) {
      console.error('Failed to fetch analytics providers', e);
      return [];
    }
  }, [token, account]);
  ...
  const fetchEcosystemProviders = useCallback(async () => {
    setLoading(true); setError(null);
    try {
      const data = await conversionTrackerApi.listEcosystemProviders(headers);
      setEcosystemProviders(data?.providers || []);
  ...
  const resyncEcosystemProvider = useCallback(async (providerId) => {
    setLoading(true); setError(null);
    try {
      const data = await conversionTrackerApi.resyncEcosystemProvider({ providerId, ...headers });
      return data;
    } catch (e) {
      setError(e);
      throw e;
  ...
  }, [autoLoad, token, account, refreshAll, fetchEcosystemProviders, fetchAnalyticsProviders]);
